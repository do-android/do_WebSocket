...
import doext.websocket.koushikdutta.http.AsyncHttpClient;
import doext.websocket.koushikdutta.http.AsyncHttpClient.SocketIORequest;
...
class SocketIOConnection {
		...
		httpClient.executeString(request, new AsyncHttpClient.StringCallback() {
			...
			public void onCompleted(final Exception e, String result) {
				...
				try {
					...
					final String sessionUrl = Uri.parse(request.getUri()).buildUpon().appendPath("websocket").appendPath(session).build().toString();

					SocketIOConnection.this.webSocketClient = new WebSocketClient(URI.create(sessionUrl), new WebSocketListener() {
	...
	void setupHeartbeat() {
		...
		Runnable heartbeatRunner = new Runnable() {
			@Override
			public void run() {
				if (heartbeat <= 0 || ws != webSocketClient || ws == null || !ws.isConnected())
					return;
				webSocketClient.send("2:::");
